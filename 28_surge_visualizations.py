...
def use_black_gray_with_different_line_styles_to_dis(X, X_test, logger, model, y_test) -> None:
    ...
    for v, style, label in zip(velocities, styles, labels):
        probe = pd.DataFrame({'linepack': 1.0, 'closure_time_s': closure_grid, 'pump_trip': 0, 'velocity_ms': v, 'elevation_drop_m': 0, 'temperature_c': 15})
        predicted = model.predict(probe)
        ax.plot(closure_grid, predicted, color='#2b2b2b', linestyle=style, linewidth=2, label=label, alpha=0.85)
    ax.axhline(y=260, color='#d62728', linestyle='-', linewidth=2, label='MAOP (260 psig)', alpha=0.8)
    ...
    ax.annotate('Safe Zone\n(Below MAOP)', xy=(8, 230), fontsize=9, color='#a0a0a0', ha='center', style='italic', alpha=0.7)
    plt.tight_layout()
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    bars = ax.bar(range(len(importances)), importances[indices], color='#ffffff', edgecolor='#2b2b2b', linewidth=1.5, alpha=0.9)
    ...
    for i, bar in enumerate(bars):
        ...
        ax.text(bar.get_x() + bar.get_width() / 2, height + 0.01, f'{importances[indices[i]]:.1%}', ha='center', va='bottom', fontsize=9)
    signalplot.tidy_axes(ax)
    plt.tight_layout()
...
def prepare_base() -> None:
    ...
    df = pd.DataFrame({'linepack': linepack, 'closure_time_s': closure_time, 'pump_trip': pump_trip, 'velocity_ms': velocity, 'elevation_drop_m': elev, 'temperature_c': temp, 'peak_overpress_psig': peak_overpress})
    ...
    model = Pipeline([('scaler', StandardScaler()), ('gbr', GradientBoostingRegressor(max_depth=5, n_estimators=500, learning_rate=0.05, random_state=42))])
